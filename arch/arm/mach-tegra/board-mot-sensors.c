...
static int kxtf9_gpio_level(void)
{
	return gpio_get_value(TEGRA_KXTF9_INT_GPIO);
}

static int etna_kxtf9_gpio_level(void)
{
	return gpio_get_value(TEGRA_KXTF9_INT_GPIO_ETNA_S1);
}
...
static void __init kxtf9_init(void)
{
    ...
    if (machine_is_tegra_daytona()) {
        kxtf9_data.gpio = kxtf9_gpio_level;
        ...
        if (system_rev == 0x1100)
            kxtf9_data.gpio = etna_kxtf9_gpio_level;
    ...
    if (machine_is_tegra_daytona()) {
    	...
    	gpio_request(TEGRA_KXTF9_INT_GPIO_ETNA_S1, "kxtf9 accelerometer int");
    	gpio_direction_input(TEGRA_KXTF9_INT_GPIO_ETNA_S1);
    }
