...
static struct usbnet_device *_usbnet_dev;
...
static inline struct usbnet_device *usb_func_to_dev(struct usb_function *f)
{
				...
				struct usbnet_context *context)
{
	...
	ret = usb_ep_queue(context->bulk_out, req, GFP_ATOMIC);
	if (ret == 0)
		return 0;
	else
		kfree_skb(skb);
...
static int usb_ether_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	rc = usb_ep_queue(context->bulk_in, req, GFP_ATOMIC);
	if (rc != 0) {
		spin_lock_irqsave(&context->lock, flags);
...
static void usb_ether_setup(struct net_device *dev)
{
	...
	dev->netdev_ops = &usb_eth_netdev_ops;
...
{
	struct usbnet_device *dev = usb_func_to_dev(f);
...
static int usbnet_setup(struct usb_composite_dev *cdev,
