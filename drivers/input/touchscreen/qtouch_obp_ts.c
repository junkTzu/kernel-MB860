...
static int qtouch_hw_init(struct qtouch_ts_data *ts)
{
	...
	if (obj && obj->entry.num_inst > 0) 
	{
		ret = qtouch_write_addr(ts, obj->entry.addr,
					...
					min(sizeof(ts->pdata->grip_face_suppression_cfg),
					    obj->entry.size));
		if (ret != 0) 
		{
...
*/
	...
	case QTOUCH_IOCTL_SET_OBJECT:
		...
		case QTM_OBJ_PROCI_GRIPFACESUPPRESSION:
			...
			/* Each bit represents a field. Max of 32 fields */
			for ( i = 0; i < sizeof(struct qtm_proci_grip_face_suppression_cfg); i++ )
			{
			...
			memcpy((char *)&(tsGl->pdata->grip_face_suppression_cfg),(char *)fields,sizeof(struct qtm_proci_grip_face_suppression_cfg));
			break;
