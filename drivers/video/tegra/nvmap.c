...
static struct tegra_iovmm_area *_nvmap_get_vm(struct nvmap_handle *h)
{
	...
	vm = tegra_iovmm_create_vm(nvmap_vm_client, NULL, h->size, h->align,
		_nvmap_flag_to_pgprot(h->flags, pgprot_kernel), h->pgalloc.iovm_addr);
...
//Ugly hack to user newer tegra_iovmm_alloc_client
static int nvmap_open(struct inode *inode, struct file *filp)
{
	/* eliminate read, write and seek support on this node */
	...
	unsigned long start, unsigned long end, unsigned long op, bool get)
{
	...
	if (op == NVMEM_CACHE_OP_WB) {
		//inner_maint = dmac_clean_range; No longer used
		...
		//inner_maint = dmac_inv_range; No longer used
		if (h->flags == NVMEM_HANDLE_CACHEABLE)
